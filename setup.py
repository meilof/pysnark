...
if disable_qaptools and disable_libsnark:  
    ...
    my_exts=[] + ([CMakeExtension("pysnark.libsnark._libsnark", cmake_lists_dir="depends/python-libsnark")] if not disable_libsnark else []) + ([CMakeExtension("pysnark.qaptools.allx", cmake_lists_dir="depends/qaptools", prefun=use_qaptools_bins if not qaptools_bin is None else None)] if not disable_qaptools else [])
