...
def next_curve_point(file):
    ...
    line = next(file)
    return EcPt.from_binary(bytes.fromhex(line), group)
...
class CommittedValue:
    ...
    def __add__(self, other):
        return CommittedValue(self.gi+other.gi, 
                              (self.vi+other.vi) % int(group.order()),
                              (self.ri+other.ri) % int(group.order()))
    ...
    def __mul__(self, other):
        return CommittedValue(other*self.gi, 
                              (other*self.vi) % int(group.order()),
                              (other*self.ri) % int(group.order()))

    __rmul__ = __mul__

    def __neg__(self):
        return CommittedValue(-self.gi, 
                              int(group.order())-self.vi,
                              int(group.order())-self.ri)
